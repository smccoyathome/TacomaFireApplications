...
namespace PTSProject
{
	...
	public partial class frmTrainReqReading
		: UpgradeHelpers.Helpers.FormBase<PTSProject.ViewModels.frmTrainReqReadingViewModel>, UpgradeHelpers.Interfaces.IInitializable
	{

		void UpgradeHelpers.Interfaces.IInitializable.Init()
		{
			this.CallBaseInit(typeof(frmTrainReqReading));
		...
		private void frmTrainReqReading_Activated(System.Object eventSender, System.EventArgs eventArgs)
		{
		...
		//*************************************************************
		//Training Required Reading Quarterly Report
		...
		public void FormatHeadings2()
		{
			...
			sHeading = "Required Reading Assignments For ";
			...
			if (TrainCL.GetTrainingReqReadingByYearType(ViewModel.CurrYear, ViewModel.CurrType) != 0)
			{
				//continue
			}
			else
			{
				ViewManager.ShowMessage("Oh No!  There are no Training Required Reading Assignments set up for Year/Type!", "Training Minimum Standard Drill Report", UpgradeHelpers.Helpers.BoxButtons.OK);
				return;
			}
			...
			while(!TrainCL.TrainingRecord.EOF)
			{
				...
				if (sSubHeading == "")
				{ //first time
					sSubHeading = modGlobal.Clean(TrainCL.TrainingRecord["group_description"]);
				...
				else if (sSubHeading == modGlobal.Clean(TrainCL.TrainingRecord["group_description"]))
				{
					ViewModel.sprReport.Row = iCurrRow + 1;
		...
		public void FormatReport()
		{
			...
			if (TrainCL.GetTrainingQuarterlyReqReadingReport(ViewModel.CurrYear, ViewModel.CurrQuarter, ViewModel.CurrBatt, ViewModel.CurrShift) != 0)
			{
				//continue
			}
			else
		...
		public void FormatReport2()
		{
			...
			if (TrainCL.GetTrainingAnnualReqReadingReportByType(ViewModel.CurrYear, ViewModel.CurrType, ViewModel.CurrBatt, ViewModel.CurrShift) != 0)
			{
				//continue
			}
			else
		...
		public void FormatHeadings()
		{
			...
			if (TrainCL.GetTrainingReqReadingByYearQuarter(ViewModel.CurrYear, ViewModel.CurrQuarter) != 0)
			{
				//continue
			}
			else
			{
				ViewManager.ShowMessage("Oh No!  There are no Training Required Reading Assignments set up for Year/Quarter!", "Training Quarterly Report", UpgradeHelpers.Helpers.BoxButtons.OK);
				return;
			}
		...
		public void LoadLists()
		{
			...
			if (TrainCL.GetTrainingReadingGroupList() != 0)
			{

				while(!TrainCL.TrainingRecord.EOF)
				{
					ViewModel.cboType.AddItem(modGlobal.Clean(TrainCL.TrainingRecord["group_description"]));
					//UPGRADE_WARNING: (1068) GetVal() of type Variant is being forced to int. More Information: http://www.vbtonet.com/ewis/ewi1068.aspx
					ViewModel.cboType.SetItemData(ViewModel.cboType.GetNewIndex(), Convert.ToInt32(modGlobal.GetVal(TrainCL.TrainingRecord["group_id"])));
					TrainCL.TrainingRecord.MoveNext();
				};
			}
		...
		internal void cmdPrint_Click(Object eventSender, System.EventArgs eventArgs)
		{
			...
			ViewModel.sprReport.setPrintAbortMsg("Printing Training Required Reading Report");
		...
		public static frmTrainReqReading DefInstance
		{
			get
			{
				if ( Shared.m_vb6FormDefInstance == null)
		...
		public static frmTrainReqReading CreateInstance()
		{
			PTSProject.frmTrainReqReading theInstance = Shared.Container.Resolve< frmTrainReqReading>();
		...
		: UpgradeHelpers.Helpers.FormBase<PTSProject.ViewModels.frmTrainReqReadingViewModel>, UpgradeHelpers.Interfaces.IInitializable
	{
			...
			Interfaces.IInteractsWithView, UpgradeHelpers.Interfaces.IInitializableCommon, UpgradeHelpers.Interfaces.IInitializable
		{
			...
			public virtual frmTrainReqReading m_vb6FormDefInstance { get; set; }
