...
namespace PTSProject
{
	...
	public partial class frmDispatchScheduler
		: UpgradeHelpers.Helpers.FormBase<PTSProject.ViewModels.frmDispatchSchedulerViewModel>, UpgradeHelpers.Interfaces.IInitializable
	{

		void UpgradeHelpers.Interfaces.IInitializable.Init()
		{
			this.CallBaseInit(typeof(frmDispatchScheduler));
		...
		public UpgradeHelpers.Helpers.ControlViewModel GetFullShift(UpgradeHelpers.Helpers.ControlViewModel DropCntrl)
		{
			...
			switch ( DropCntrl.Name )
			{
				...
				case "lbPos7pm":
					...
					ViewModel.bAMOnly = false;
					// Missing lbPos8am... AM shift only 
					break;
				case "lbPos9am":
		...
		public void LabelDragDrop(UpgradeHelpers.Helpers.ControlViewModel DropCntrl, UpgradeHelpers.Helpers.ControlViewModel SourceCntrl)
		{
			using ( var async1 = this.Async(true) )
			{
				...
				string OldDate = "";
				int ControlID = 0;
				UpgradeHelpers.Helpers.ControlViewModel ShiftControl = null;
				ViewModel.bAMOnly = false;
				...
				if ( !ViewModel.bAMOnly )
				{ // bAMOnly = False
					ShiftControl = GetFullShift(DropCntrl);
				...
				if ( SourceCntrl.Name == "pnSelected" )
				{
                    ...
                    async1.Append<PTSProject.frmDispatchScheduler.ScheduleEmployeeStruct>(() => ScheduleEmployee(
                                ControlID, ShiftDate, tempRefParam, ViewModel.lbShiftArray[UpgradeHelpers.Helpers.ControlArrayHelper.GetControlIndex(DropCntrl)].Text));
					async1.Append<PTSProject.frmDispatchScheduler.ScheduleEmployeeStruct>(tempNormalized1 =>
						{
							var returningMetodValue252 = tempNormalized1;
						
							ShiftDate = returningMetodValue252.ShiftDate;
							tempRefParam = returningMetodValue252.Empid;
							SourceCntrl.Text = "";
				...
				else
				{
					...
					if ( dropAMPM == srcAMPM )
					{
						if ( UpgradeHelpers.Helpers.ControlArrayHelper.GetControlIndex(DropCntrl) == UpgradeHelpers.Helpers.ControlArrayHelper.GetControlIndex(SourceCntrl) )
						{
                            ...
                            async1.Append<PTSProject.frmDispatchScheduler.MoveEmployeeStruct>(() => MoveEmployee(ControlID, ShiftDate,
                                    tempRefParam2, ViewModel.lbShiftArray[UpgradeHelpers.Helpers.ControlArrayHelper.GetControlIndex(DropCntrl)].Text));
							async1.Append<PTSProject.frmDispatchScheduler.MoveEmployeeStruct>(tempNormalized3 =>
								{
									var returningMetodValue260 = tempNormalized3;
								
									ShiftDate = returningMetodValue260.ShiftDate;
									tempRefParam2 = returningMetodValue260.Empid;
								});
						}
						else
						{
							...
							async1.Append<PTSProject.frmDispatchScheduler.ReScheduleEmployeeStruct>(() => ReScheduleEmployee(
							...
							async1.Append<PTSProject.frmDispatchScheduler.ReScheduleEmployeeStruct>(tempNormalized5 =>
								{
									var returningMetodValue267 = tempNormalized5;
									if ( returningMetodValue267.returnValue )
									{


										ShiftDate = returningMetodValue267.ShiftDate;
										tempRefParam3 = returningMetodValue267.Empid;
										OldDate = returningMetodValue267.OldDate;
										SourceCntrl.Text = "";
										UpgradeHelpers.Helpers.ControlHelper.SetTag(SourceCntrl, "");
							...
							async1.Append<MoveEmployeeStruct>(() =>
								{
									...
									{
										return this.Return<MoveEmployeeStruct>(() => new PTSProject.frmDispatchScheduler.MoveEmployeeStruct()
		...
		public DeleteScheduleStruct DeleteSchedule(string Empid, string ShiftDate)
		{
                ...
                if (modGlobal.ErrorControl() == modGlobal.eFATALERROR)
						{
                    return new PTSProject.frmDispatchScheduler.DeleteScheduleStruct()
				...
				{
					{
						...
						if ( !oRec.EOF )
						{
							...
							return this.Return<ScheduleEmployeeStruct>(() => new PTSProject.frmDispatchScheduler.ScheduleEmployeeStruct()
                                { ShiftDate = ShiftDate, Empid = Empid, });
						}
								...
								using ( var async2 = this.Async() )
								{
									...
									if ( modGlobal.Shared.gFullShift != 0 )
									{
										...
										if ( !oRec.EOF )
										{
											...
											{
                                            var returningMetodValue431 = DeleteSchedule(Empid, ShiftDate);
											Empid = returningMetodValue431.Empid;
											ShiftDate = returningMetodValue431.ShiftDate;
											oCmdInsert.ExecuteNonQuery(new object[] { ShiftDate, Empid, AssignID, TimeCode, PayUp, JobCode, Step, DateTime.Now, modGlobal.Shared.gUser });
											}
										}
										else
				...
				{
					{
						...
						{
							ViewManager.ShowMessage("This Employee is already scheduled for this date", "Weekly Scheduler", UpgradeHelpers.Helpers.BoxButtons.OK, UpgradeHelpers.Helpers.BoxIcons.Exclamation);
                            return this.Return< ReScheduleEmployeeStruct>(() => new PTSProject.frmDispatchScheduler.ReScheduleEmployeeStruct() {
									...
									async2.Append(() =>
										{
                                            ...
                                            var returningMetodValue471 = //Delete old Schedule record
                                     DeleteSchedule(Empid, OldDate);
												Empid = returningMetodValue471.Empid;
												OldDate = returningMetodValue471.ShiftDate;
												//Add new schedule record
												oCmdInsert.Connection = modGlobal.oConn;
												oCmdInsert.CommandType = CommandType.StoredProcedure;
												oCmdInsert.CommandText = "spInsertSchedule";
												...
												if ( modGlobal.Shared.gFullShift != 0 )
												{
													...
													{
                                                    var returningMetodValue495 = DeleteSchedule(Empid, OldDate);
													Empid = returningMetodValue495.Empid;
													OldDate = returningMetodValue495.ShiftDate;
													oCmdInsert.ExecuteNonQuery(new object[] { ShiftDate, Empid, AssignID, TimeCode, PayUp, JobCode, Step, DateTime.Now, modGlobal.Shared.gUser });
													}
												}
                        ...
                        return this.Return< ReScheduleEmployeeStruct>(() => new PTSProject.frmDispatchScheduler.ReScheduleEmployeeStruct() {
                            returnValue = true, ShiftDate = ShiftDate, Empid = Empid, OldDate = OldDate, });
					}
				}
				catch
				{
		...
		internal void frmDispatchScheduler_Activated(Object eventSender, System.EventArgs eventArgs)
		{
		...
		//UPGRADE_WARNING: (2065) Form event frmDispatchScheduler.Deactivate has a new behavior. More Information: http://www.vbtonet.com/ewis/ewi2065.aspx
		[UpgradeHelpers.Events.Handler]
		internal void frmDispatchScheduler_Deactivate(Object eventSender, System.EventArgs eventArgs)
		{
						//Call Global Refresh Form subroutine
						//to refresh displayed data on open forms
									...
									using ( var async3 = this.Async() )
										{
											...
											if ( modGlobal.Shared.gFullShift != 0 )
											{
                                                ...
                                                async3.Append<PTSProject.modGlobal.ScheduleLeaveStruct>(tempNormalized3 =>
													{
                                                        var returningMetodValue9 = tempNormalized3;
													

														Response = returningMetodValue9.returnValue;

														tempRefParam3 = returningMetodValue9.Empid;

														tempRefParam4 = returningMetodValue9.LeaveDate;
														if ( Response != 0 )
														{
				...
				async1.Append(() =>
					{
                        ...
                        var returningMetodValue1894 = DeleteSchedule(Empid, ShiftDate);
										Empid = returningMetodValue1894.Empid;
										ShiftDate = returningMetodValue1894.ShiftDate;
										...
										if ( modGlobal.Shared.gFullShift != 0 )
										{
											...
											ShiftDate = ViewModel.lbWeekDay[UpgradeHelpers.Helpers.ControlArrayHelper.GetControlIndex(ViewModel.SelectedLabel)].Text + " 7:00" + AMPM;
                            var returningMetodValue1896 = DeleteSchedule(Empid, ShiftDate);
													Empid = returningMetodValue1896.Empid;
													ShiftDate = returningMetodValue1896.ShiftDate;
													ViewModel.SelectedLabel.Text = "";
													AltShift.Text = "";
													UpgradeHelpers.Helpers.ControlHelper.SetTag(ViewModel.SelectedLabel, "");
													UpgradeHelpers.Helpers.ControlHelper.SetTag(AltShift, "");
...
frmTransReport.DefInstance);
		...
		internal void picTrash_DragDrop(Object eventSender, UpgradeHelpers.Events.DragEventArgs eventArgs)
		{
                ...
                var returningMetodValue1941 = DeleteSchedule(Empid, ShiftDate);
								Empid = returningMetodValue1941.Empid;
								ShiftDate = returningMetodValue1941.ShiftDate;
										GetSchedule();
							}
...
frmTrainQuery.DefInstance);
		...
		public static frmDispatchScheduler DefInstance
		{
			get
			{
					if ( Shared.m_vb6FormDefInstance == null )
		...
		public static frmDispatchScheduler CreateInstance()
		{
				PTSProject.frmDispatchScheduler theInstance = Shared.Container.Resolve<frmDispatchScheduler>();
		...
		: UpgradeHelpers.Helpers.FormBase<PTSProject.ViewModels.frmDispatchSchedulerViewModel>, UpgradeHelpers.Interfaces.IInitializable
	{
			...
			Interfaces.IInteractsWithView, UpgradeHelpers.Interfaces.IInitializableCommon, UpgradeHelpers.Interfaces.IInitializable
		{
			...
			public virtual frmDispatchScheduler m_vb6FormDefInstance { get; set; }
