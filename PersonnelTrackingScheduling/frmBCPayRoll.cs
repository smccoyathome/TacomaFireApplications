...
namespace PTSProject
{
	...
	public partial class frmBCPayRoll
		: UpgradeHelpers.Helpers.FormBase<PTSProject.ViewModels.frmBCPayRollViewModel>, UpgradeHelpers.Interfaces.IInitializable
	{

		void UpgradeHelpers.Interfaces.IInitializable.Init()
		{
			this.CallBaseInit(typeof(frmBCPayRoll));
		...
		private void frmBCPayRoll_Activated(System.Object eventSender, System.EventArgs eventArgs)
		{
		...
		public void ProcessDateChange()
		{
			...
			if ( ViewModel.dtReportDate.Value.Date.ToString("M/d/yyyy") == ViewModel.ReportDate )
			{
				return ;
			}
			ViewModel.ReportDate = ViewModel.dtReportDate.Value.Date.ToString("M/d/yyyy");

			oCmd.CommandText = "spSelect_PayRollYearPayPeriod '" + ViewModel.ReportDate + "' ";
			ADORecordSetHelper oRec = ADORecordSetHelper.Open(oCmd, "");
			if ( !oRec.EOF )
			{
				if ( Convert.ToDouble(oRec["calendar_year"]) != modGlobal.Shared.gPayrollYear )
				{
					...
					ocmd2.CommandText = "sp_GetPayRollPeriods " + modGlobal.Shared.gPayrollYear.ToString();
					orec2 = ADORecordSetHelper.Open(ocmd2, "");

					while ( !orec2.EOF )
					{
						ViewModel.cboPayPeriod.AddItem(Convert.ToString(orec2["start_date"]) + " - " + Convert.ToString(orec2["end_date"]));
						ViewModel.cboPayPeriod.SetItemData(ViewModel.cboPayPeriod.GetNewIndex(), Convert.ToInt32(orec2["pay_period"]));
						orec2.MoveNext();
					};
					}
				}
			...
			//
			//    If Year(ReportDate) = gPayrollYear Then
			string strSQL = "SELECT max(pay_period) as pay_period FROM PayRoll_PayPeriod WHERE calendar_year = ";
			strSQL = strSQL + modGlobal.Shared.gPayrollYear.ToString() + " and datediff(day,start_date,'" + ViewModel.ReportDate + "') >= 0 ";
			...
			if ( !oRec.EOF )
			{
				...
				modGlobal
					.Shared.gPayPeriod = Convert.ToInt32(modGlobal.GetVal(oRec["pay_period"]));
				for ( int i = 0; i <= ViewModel.cboPayPeriod.Items.Count - 1; i++ )
				{
					if ( modGlobal.Shared.gPayPeriod == ViewModel.cboPayPeriod.GetItemData(i) )
					{
						ViewModel.cboPayPeriod.SelectedIndex = -1;
		...
		public void RefreshEmployeeList()
		{
			...
			oCmd.Connection = modGlobal.oConn;
			oCmd.CommandType = CommandType.Text;
			sStringText = "spSelect_BCEmployeeGridListNew ";
			...
			if ( modGlobal.Clean(ViewModel.CurrBatt) == "" )
			{
				if ( modGlobal.Clean(ViewModel.CurrUnit) == "" )
				{
					...
					{
						sStringText = sStringText + "Null, Null, Null";
					}
					else
					{
						sStringText = sStringText + "Null, Null, '" + modGlobal.Clean(ViewModel.CurrShift) + "' ";
					}
				}
				else
				{
					if ( modGlobal.Clean(ViewModel.CurrShift) == "" )
					{
						sStringText = sStringText + "Null, '" + modGlobal.Clean(ViewModel.CurrUnit) + "', Null";
					}
					else
					{
						sStringText = sStringText + "Null, '" + modGlobal.Clean(ViewModel.CurrUnit) + "', '" + modGlobal.Clean(ViewModel.CurrShift) + "' ";
					}
				}
			}
			else
			{
				if ( modGlobal.Clean(ViewModel.CurrUnit) == "" )
				{
					if ( modGlobal.Clean(ViewModel.CurrShift) == "" )
					{
						sStringText = sStringText + "'" + modGlobal.Clean(ViewModel.CurrBatt) + "', Null, Null";
					}
					else
					{
						sStringText = sStringText + "'" + modGlobal.Clean(ViewModel.CurrBatt) + "', Null, '" + modGlobal.Clean(ViewModel.CurrShift) + "' ";
					}
				}
				else
				{
					if ( modGlobal.Clean(ViewModel.CurrShift) == "" )
					{
						sStringText = sStringText + "'" + modGlobal.Clean(ViewModel.CurrBatt) + "', '" + modGlobal.Clean(ViewModel.CurrUnit) + "', Null";
					}
					else
					{
						sStringText = sStringText + "'" + modGlobal.Clean(ViewModel.CurrBatt) + "', '" + modGlobal.Clean(ViewModel.CurrUnit) + "', '" + modGlobal.Clean(ViewModel.CurrShift) + "' ";
					}
				}
			}

			sStringText = sStringText + ", " + modGlobal.Shared.gPayrollYear.ToString() + "," + modGlobal.Shared.gPayPeriod.ToString() + ", '" + ViewModel.ReportDate + "' ";
		...
		internal void cboNameList_SelectionChangeCommitted(Object eventSender, System.EventArgs eventArgs)
		{
			...
			System.DateTime TempDate = DateTime.FromOADate(0);
			modGlobal.Shared.gStartTrans = (DateTime.TryParse(ViewModel.ReportDate, out TempDate)) ? TempDate.ToString("M/d/yyyy") : ViewModel.ReportDate;
		...
		[UpgradeHelpers.Events.Handler]
		internal void cmdAllPayroll_Click(Object eventSender, System.EventArgs eventArgs)
		{

			//    MsgBox "This Feature is coming soon :-)", vbOKOnly, "Review All Battalion Timecards"
			ViewManager.NavigateToView(
...
frmPayroll.DefInstance);
		...
		internal void cmdNotes_Click(Object eventSender, System.EventArgs eventArgs)
		{
			...
			modGlobal
				.Shared.gNoteDate = ViewModel.dtReportDate.Value.Date.ToString("M/d/yyyy");
				if ( UpgradeHelpers.Helpers.StringsHelper.ToDoubleSafe(modGlobal.Shared.gPayRollBatt) == 1 )
				{
				ViewManager.NavigateToView(
					frmNote.DefInstance);
				...
				else if ( UpgradeHelpers.Helpers.StringsHelper.ToDoubleSafe(modGlobal.Shared.gPayRollBatt) == 2 )
				{
				ViewManager.NavigateToView(
		...
		private void Form_Load()
		{
			...
			ClearFields();
			ViewModel.CurrBatt = modGlobal.Shared.gPayRollBatt;
			ViewModel.CurrShift = "";
			System.DateTime TempDate = DateTime.FromOADate(0);
			ViewModel.ReportDate = (DateTime.TryParse(modGlobal.Shared.gStartTrans, out TempDate)) ? TempDate.ToString("M/d/yyyy") : modGlobal.Shared.gStartTrans;
			System.DateTime TempDate2 = DateTime.FromOADate(0);
			ViewModel.dtReportDate.Value = DateTime.Parse((DateTime.TryParse(ViewModel.ReportDate, out TempDate2)) ? TempDate2.ToString("M/d/yyyy") : ViewModel.ReportDate);
			...
			//    Else
			ViewModel.SpecialEndDate = DateTime.Parse(DateTime.Now.ToString("MM/dd/yyyy"));
			//        SpecialEndDate = Format$(ReportDate, "m/d/yyyy")
			//    End If
			...
			if ( modGlobal.Shared.gPayRollBatt == "3" )
			{
				ViewModel.CurrBatt = "3";
			}
			else if ( modGlobal.Shared.gPayRollBatt == "2" )
			{
			...
			this.ViewModel.Text = "Battalion " + modGlobal.Shared.gPayRollBatt + " Payroll";
			FillLists();
			LoadLists();
		...
		public static frmBCPayRoll DefInstance
		{
			get
			{
				if ( Shared.m_vb6FormDefInstance == null )
		...
		public static frmBCPayRoll CreateInstance()
		{
			PTSProject.frmBCPayRoll theInstance = Shared.Container.Resolve<frmBCPayRoll>();
		...
		: UpgradeHelpers.Helpers.FormBase<PTSProject.ViewModels.frmBCPayRollViewModel>, UpgradeHelpers.Interfaces.IInitializable
	{
			...
			Interfaces.IInteractsWithView, UpgradeHelpers.Interfaces.IInitializableCommon, UpgradeHelpers.Interfaces.IInitializable
		{
			...
			public virtual frmBCPayRoll m_vb6FormDefInstance { get; set; }
