...
namespace PTSProject
{
	...
	public partial class frmPayrollBCStaff
		: UpgradeHelpers.Helpers.FormBase<PTSProject.ViewModels.frmPayrollBCStaffViewModel>, UpgradeHelpers.Interfaces.IInitializable
	{

		void UpgradeHelpers.Interfaces.IInitializable.Init()
		{
			this.CallBaseInit(typeof(frmPayrollBCStaff));
		...
		private void frmPayrollBCStaff_Activated(System.Object eventSender, System.EventArgs eventArgs)
		{
		...
		public void FormatTimeCard()
		{
			...
			if ( !oRec.EOF )
			{
				...
				ViewModel.CurrEmpSAPID = Convert.ToInt32(oRec["sap_id"]);
			}
			else
			{
				ViewModel.sprTimeSheet.Text = "00000000";
		...
		public bool GetEmployeePayRoll()
		{
			...
			while ( !oRec.EOF )
			{
				...
				else
				{
					...
					//COME BACK HERE
					if ( modGlobal.Clean(ViewModel.CurrApproveDate) != "" )
					{
						if ( String.CompareOrdinal(modGlobal.Clean(oRec["last_update_date"]), ViewModel.CurrApproveDate) > 0 )
						{
							ViewModel.lbApprovedOn.Text = "Payroll has changed since Timecard Approval";
						}
					}
		...
		public void FillTimeCard()
		{
			...
			{
			//        if lbApprovedOn.Caption = "Timecard Approved On: " & Clean(sprEmployeeList.Text)
			}
		...
		public void RefreshEmployeeList()
		{
			...
			oCmd.Connection = modGlobal.oConn;
			oCmd.CommandType = CommandType.Text;
			sStringText = "spSelect_BCStaffGridList ";
			...
			while ( !oRec.EOF )
			{
				...
				if ( modGlobal.Clean(oRec["ApprovalFlag"]) == "N" )
				{
				...
				if ( modGlobal.Clean(oRec["OKflag"]) == "Y" )
				{
					NoSchedule = false;
					NoLeave = false;
					NoExceptions = false;
				...
				ViewModel.sprEmployeeList.Text = modGlobal.Clean(oRec["approve_datetime"]);
		...
		internal void cmdApply1_Click(Object eventSender, System.EventArgs eventArgs)
		{
			...
			if ( modGlobal.Clean(ViewModel.sprWeek1.Text) != modGlobal.Clean(ViewModel.txtFr1.Text) || RecordChange || ViewModel.sprWeek1.ForeColor.Equals(modGlobal.Shared.BLUE) )
			{
				...
				{
					PayrollCL.PPsap_acttype = modGlobal.Clean(ViewModel.txtActivity1.Text);
					PayrollCL.PPsap_rec_center = modGlobal.Clean(ViewModel.txtCostCenter1.Text);
					PayrollCL.PPwbs_element = modGlobal.Clean(ViewModel.txtWBSElement1.Text);
					PayrollCL.PPsap_rec_order = modGlobal.Clean(ViewModel.cboOrder1.Text);
					PayrollCL.PPsap_activity = modGlobal.Clean(ViewModel.txtOper1.Text);
		...
		{
			PTSProject.clsFireUpload PayrollCL = Container.Resolve<clsFireUpload>();
			//    MsgBox "This Feature is coming soon!", vbOKOnly, "Approve Timecard"
			ViewModel.cmdApprove.Enabled = false;

			if ( ViewModel.CurrEmpSAPID == 0 )
			{
				ViewManager.ShowMessage("Something is wrong! Call Debra Lewandowsky and tell her that there was no SAP ID For Employee!! ", "No SAP ID For Employee", UpgradeHelpers.Helpers.BoxButtons.OK);

				return ;
			}

			if ( PayrollCL.GetSupervisorTCApproval(modGlobal.Shared.gPayrollYear, modGlobal.Shared.gPayPeriod, ViewModel.CurrEmpSAPID) != 0 )
			{
				PayrollCL.STCApprovalSupervisor = modPTSPayroll.Shared.gUserSAPid;
				if ( ~PayrollCL.UpdateSupervisorTCApproval() != 0 )
				{
					ViewManager.ShowMessage("Ooops! Something went wrong, record was not updated.", "Update Timecard Approval Record", UpgradeHelpers.Helpers.BoxButtons.OK);
					ViewModel.cmdApprove.Enabled = true;
					ViewModel.lbApprovedOn.Text = "";
				}
				else
				{
					ViewModel.lbApprovedOn.Text = "Timecard was approved On " + DateTime.Now.ToString("MM/dd/yyyy");
				}
			}
			else
			{
				PayrollCL.STCApprovalcalendar_year = modGlobal.Shared.gPayrollYear;
				PayrollCL.STCApprovalpay_period = modGlobal.Shared.gPayPeriod;
				PayrollCL.STCApprovalEmployee = ViewModel.CurrEmpSAPID;
				PayrollCL.STCApprovalSupervisor = modPTSPayroll.Shared.gUserSAPid;
				PayrollCL.STCApprovaldate = UpgradeHelpers.Helpers.DateTimeHelper.ToString(DateTime.Now);
				if ( ~PayrollCL.AddSupervisorTCApproval() != 0 )
				{
					ViewManager.ShowMessage("Ooops! Something went wrong, record was not inserted.", "Update Timecard Approval Record", UpgradeHelpers.Helpers.BoxButtons.OK);
					ViewModel.cmdApprove.Enabled = true;
					ViewModel.lbApprovedOn.Text = "";
				}
				else
				{
					ViewModel.lbApprovedOn.Text = "Timecard was approved On " + DateTime.Now.ToString("MM/dd/yyyy HH:mm:ss");
					ViewModel.sprEmployeeList.Col = 1;
					ViewModel.sprEmployeeList.Value = 1;
				}
			}
		...
		private void sprEmployeeList_CellClick(object eventSender, Stubs._FarPoint.Win.Spread.CellClickEventArgs eventArgs)
		{
			...
			{
				ViewModel.CurrApproveDate = modGlobal.Clean(ViewModel.sprEmployeeList.Text);
				ViewModel.lbApprovedOn.Text = "Timecard Approved On: " + modGlobal.Clean(ViewModel.sprEmployeeList.Text);
			}
		...
		public static frmPayrollBCStaff DefInstance
		{
			get
			{
				if ( Shared.m_vb6FormDefInstance == null )
		...
		public static frmPayrollBCStaff CreateInstance()
		{
			PTSProject.frmPayrollBCStaff theInstance = Shared.Container.Resolve<frmPayrollBCStaff>();
		...
		: UpgradeHelpers.Helpers.FormBase<PTSProject.ViewModels.frmPayrollBCStaffViewModel>, UpgradeHelpers.Interfaces.IInitializable
	{
			...
			Interfaces.IInteractsWithView, UpgradeHelpers.Interfaces.IInitializableCommon, UpgradeHelpers.Interfaces.IInitializable
		{
			...
			public virtual frmPayrollBCStaff m_vb6FormDefInstance { get; set; }
