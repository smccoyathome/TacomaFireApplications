...
namespace SimmoTech.Utils.Serialization
{
	...
	public sealed class SerializationWriter: BinaryWriter
	{
		...
		/// </summary>
		public SerializationWriter(ISurrogateContext context): this(context,new MemoryStream(DefaultCapacity)) {  }

		/// <summary>
		/// Creates a FastSerializer with the specified capacity
		/// </summary>
		/// <param name="capacity"></param>
		public SerializationWriter(ISurrogateContext context,int capacity): this(context,new MemoryStream(capacity)) {  }
		...
		public SerializationWriter(ISurrogateContext context,Stream stream): base(stream)
		{
            this.context = context;
            initialOffset = stream.Position;
		...
		public void WriteObject(object value)
		{
			...
			else if (value is IStateObject)
			{
				writeTypeCode(SerializedType.IStateObject);
				WriteOptimized((value as IStateObject).UniqueID);
			}

			else
			{
				var surrogate = context.GetStateObjectSurrogate(newValueObject:value, generateIfNotFound: false);
				if (surrogate != null)
				{
					writeTypeCode(SerializedType.Surrogate);
					WriteOptimized(((IStateObject)surrogate).UniqueID);
				}
				else
				{
	...
	public sealed class SerializationReader: BinaryReader
	{
        ...
        #endregion Static
        #region Constructor
        readonly ISurrogateContext context;
		...
		public SerializationReader(ISurrogateContext context,byte[] data): this(context,new MemoryStream(data)) {}
        ...
        public SerializationReader(ISurrogateContext context,Stream stream): base(stream)
		{
            this.context = context;
			...
			stream.Position = 4 + startPosition;
		}
		...
		public IStateObject ReadOptimizedIStateObject()
		{
			string uniqueID = ReadOptimizedString();
			return  context.RestoreStateObject(uniqueID);
		}

		public object ReadOptimizedSurrogate()
		{
			string uniqueID = ReadOptimizedString();
			var surrogateValue =  context.RestoreSurrogateValue(uniqueID);
		...
		private object processObject(SerializedType typeCode)
		{
			...
			else if (typeCode == SerializedType.IStateObject)
				return ReadOptimizedIStateObject();
			else if (typeCode == SerializedType.Surrogate)
				return ReadOptimizedSurrogate();
